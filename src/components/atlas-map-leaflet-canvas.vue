...
//var plottingData = []  // This singleton will be populated by a method of this component
	// It's not included in the component's data because we don't want it to be reactive 
...
export default {
	...
	mounted () {
        ...
        var chartCanvas = L.canvasOverlay(Charts.drawingOnCanvas)
        console.log(chartCanvas)
            //.drawing(Charts.drawChartFactory('pie'))
        chartCanvas.addTo(this.map)
...
/*
		calcPlottingData () {
			...
			var data = coordenadas.map(({id, lat, long}) => {
				...
				// por isso precisamos primeiro chegar se o local existe no object candidato.votos 
				var votos = candidatos.map((candidato) => candidato.votos[id] ? candidato.votos[id].numero : 0),
					...
					angulos = posicoes.map((posicao) => radianos[posicao])
